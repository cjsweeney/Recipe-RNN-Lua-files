...
cmd:option('-inDirectory', '/home/connor/Downloads/now_youre_cooking_data/train/precisetag', 'The input recipe directory')
...
local function build_recipe_matrix(counter, dict, ing_dict, title_dict)
	...
	--ing_dict is created by create_dictionary and contains the mapping from each symbol to numeric index but only for ingredients
	--	print("ing_dict test for sugar: ", ing_dict.symbol_to_index["sugar"], "and for soup:" ,ing_dict.symbol_to_index["soup"])
	--	>> ing_dict test for sugar: 	5392	and for soup:	5394

	--title_dict is created by create_dictionary and contains the mapping from each symbol to numeric index but only for titles
	--	print("title_dict test for sugar: ", title_dict.symbol_to_index["sugar"], "and for cow:" ,ing_dict.symbol_to_index["chicken"])
	--	>> title_dict test for sugar: 	1943	and for cow:	9445

	--dict is created by create_dictionary and contains the mapping from each symbol to numeric index for steps
	--	print("dict test for sugar: ", dict.symbol_to_index["sugar"], "and for cow:" ,dict.symbol_to_index["beet"])
	--	>> dict test for sugar: 	7010	and for cow:	7670
	--counter is a table with internal counters for a numbr of things, including title_length_counts, full_recipe_len_counts, ing_length_counts, ing_number_counts, nrecipes, ing_counts, cantusefull (this is a dictionary), cantuse (also a dictionary), lost_map (also a dictionary), title_counts
   
   local full_recipe_to_pos = {}
   local recipe_mat = {}
   local of_recipe_len = {}

   for length,count in pairs(counter.full_recipe_len_counts) do --length of counter.full_recipe_len_counts is 373
   ...
   for length,count in pairs(counter.title_length_counts) do --length of counter.title_length_counts is 13
   ...
   for length,count in pairs(counter.ing_length_counts) do --length of counter.ing_length_counts is 45
   ...
   for length,count in pairs(counter.ing_number_counts) do --length of counter.ing_number_counts is 62
   ...
   for recipefilename in filelist:lines() do
      ...
      for line in recipefile:lines() do
        ...
        elseif stringx.strip(line) == "" then --skips lines that are empty
        elseif stringx.startswith(line, "END") then
	...
	print("full_title: ", full_title)
               ...
               for j=1,#step_tokens do
                  ...
                  if token ~= '=' and token ~= '~' and token ~= '+' then
                     ...
                     if token ~= '' then
                        ...
                        --dbg() --entering into the Lua debugger (debugger is found in debugger.lua)
            ...
            prob_str_table = {}
            switch_prob = false
         elseif switch_prob then --prob_str_stabe skips every other line in the steps. This elseif code is only accessed if switch_prob is True, and it is only made true in the else statement below, which is only accessed if the line does not start with "END", "servings", "categories", "title", or is an empty string
