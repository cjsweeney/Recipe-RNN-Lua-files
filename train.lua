...
cmd:option('-train_data_dir', '/home/connor/Documents/outputfrom_build_mats', 'Training data directory')
cmd:option('-dev_data_dir', '', 'Dev set data directory')
cmd:option('-data_file_info', 'notco.', 'Files version info tag')
cmd:option('-checkpoint_dir', '/home/connor/Downloads/', 'output directory where checkpoints get written')
...
--   end
   	--dbg()
   	print("at this line in train_model")
...
--   for step_idx = 1, max_info.num_steps 
      ...
      for i=1,data_loader.ntrain do
        ...
        print("you made it here, right before the backward_pass")
...
local function main()
   ...
   local data_loader = RecipeDataMinibatchLoader.create(opt.train_data_dir, opt.dev_data_dir, opt.batch_size, false, opt.data_file_info, opt, true) --loads the minibatches created by build_recipe_mats_OLD.lua
